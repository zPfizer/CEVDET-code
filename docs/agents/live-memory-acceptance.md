...
## Kabul kapsamının tamamlanması

Aşağıdaki tablo kritik davranışları ve gerçek host geçişlerini tanımlar; bütün
birim testlerini ayrı canlı satırlara dönüştüren kapalı bir test listesi değildir.
Yetkili hedef tesliminden önce seçilen commitin regresyonları bu planla davranış
ailesi düzeyinde eşlenir. Kabul kaydında her aile için kaynak test, repo/Windows CI
kanıtı, hostta değişen sınır, ilgili canlı koşu ve açık boşluk gösterilir.
Deterministik yardımcı ayrıntıları CI ile doğrulanabilir; gerçek App'e bağlı
kaynak, gizlilik, yetki veya kurtarma sınırı yalnız CI sonucu ile kapatılamaz.
Eşleme tamamlanmamışsa veya böyle bir kritik sınır için canlı kanıt eksikse
kabul verilmez. İncelemede bulunan yeni sınırlar bu hedefe ait matriste izlenir;
bu belgenin merge edilmesi açık sınırları ya da koşulmamış testleri geçti saymaz.
...
| Sır olmayan prompt'un taşıma sınırı | Benzersiz, sır biçiminde olmayan bir kullanıcı prompt canary'si gerçek App'te verilir. Oluşan hookin dosyası yalnız sınırlandırılmış transcript referansı/teslim metaverisini taşır; prompt'un ikinci tam kopyası bulunmaz. Runtime user-prompt receipt'inin yolu/içeriği prompt canary'sini ve ham session kimliğini taşımaz. Ham konuşma kaynağı, izinli kaynaklı hafıza yayını ve taşıma metaverisi ayrı karşılaştırılır; unresolved taşıma girdisinde de gereksiz prompt kopyası kalmaz. |
...
| Stop sırasında geçersiz profil | Teste ait kontrollü profil, Stop'tan hemen önce geçersiz hale getirilir; normal ve read-only oturum ayrı denenir. Normal Stop en fazla bir düzeltme dönüşü ister, yinelenen Stop görünür kalıcı hatayı bildirir ve yeni block/recursive döngü üretmez. Read-only Stop düzeltme/yazma yetkisi açmadan hata bildirir; marker ve özgün profil korunur, hafıza kuyruğu/yayını başlamaz. İzinli profil onarımı sonrasında Stop ve sağlık durumu düzelir; olay, düzeltme denemesi ve önce/sonra kaynak kimliği kaydedilir. |
| UserPromptSubmit bağlamı | Seçili sorgunun gerçek prompt olayına ait `additionalContext` çıktısı kaydedilir. Önceden belirlenen izinli ilgili kaynak, güncel içerik ve provenance çıktıda doğrulanır; gizlilik süzmesi, hedef hook kaydındaki context sınırı ve tam hüküm korunur. İzinli dayanak beklendiğinde boş çıktı, eski kaynak veya süzülmemiş içerik kabul edilmez. İzinli temel kimlik/profil bağlamı önceden ayrılır; sorguyla ilgisiz, teste ait küçük bir nota benzersiz canary konur. Hem kaynak gerektiren sorguda hem Vault hatırlaması gerektirmeyen bağımsız bir prompt'ta bu ilgisiz canary ve not içeriği çıktıda bulunmaz; canary'nin bulunmaması ve yalnız beklenen kaynağın bulunması tek başına yeterli değildir. Vault hatırlaması gerektirmeyen bağımsız prompt için kişisel Vault/history retrieval'ı hiç başlamaz; hook/araç çağrı akışı bunu doğrular. Sonuçları arayıp sonradan çıktıdan çıkarmak başarı sayılmaz; önceden ayrılan temel kimlik/profil bağlamı bu arama kontrolünden ayrıdır. Teste ait bozuk/okunamayan `controls/suppressions.jsonl`, okunamayan/değişmiş kaynak veya geçersiz profil koşullarında kapalı kalma tanısı görünür olur; ham notlara ya da eski önbelleğe yönelinmez. Bu prompt'un çıktısı kendi olay kimliğiyle değerlendirilir. |
...
| Tekrarlı olay ve aynı içerikli ayrı katkılar | Aynı transcript aralığının Stop, PreCompact ve SessionEnd replay'i ek yayın üretmez. Ayrı koşuda, içeriği birebir aynı olan iki meşru ardışık batch gerçek App transcript'inde farklı konumlarla oluşturulur. Her konum bir kez işlenir ve coverage ikisinin sonuna kadar ilerler; yalnız içerik hash'iyle ikinci katkı atılmaz. Teste ait aynı transcript path'inde coverage kaydedildikten sonra, ayrı koşularda aynı uzunlukta önek değiştirilir ve transcript kısaltılır. Coverage yalnız doğrulanmış bayt/hash kimliğiyle geçerlidir; eski coverage yeni nesle taşınmaz. Yeni katkı yeniden işlenir veya görünür hatayla güvenli biçimde durur; önce/sonra kaynak kimliği, coverage ve model/yayın aralıkları eşleştirilir. Sonraki lifecycle replay'i üçüncü yayın üretmez; konum, coverage ve receipt kanıtları birlikte doğrulanır. |
...
| Açık unutma | Kullanıcı daha önce kaydedilmiş zararsız bilgiyi açık hedefle unutturur. Kaynak sessizce silinmeden ilgili türevler sonraki oturum okuması ve derleme girdisinden dışlanır; ilgisiz bilgi korunur. Kalıcı kontrol durumundaki tombstone yalnız hedef metin hash'ini taşır; hedef metin ve benzersiz hedef canary'si kontrol state'ine yazılmaz. Ayrı hata koşusunda önceden geçerli bir suppression kuralı varken yeni unutma kuralının kalıcı yazımı kontrollü başarısız kılınır. Hata görünürdür, eski ledger bayt/hash'i değişmez, önceki dışlama ve filtreli arama sonucu korunur; yeni kural başarıyla uygulanmış sayılmaz. |
...
| Sır süzme | Gerçek kimlik bilgisi kullanmadan, en az `DATABASE_PASSWORD`, `MY_TOKEN` ve `AWS_SECRET_ACCESS_KEY` atamalarının her birine farklı benzersiz sentetik canary değeri verilerek gerçek App oturumuna girilir; kolay tanınan tek bir `api_key` örneği yeterli değildir. Ayrı gerçek konuşma ve App ek-dosyası koşularında tırnaklı `password`/`authorization` alanları, nesne/liste değerleri, kaçışlı anahtar/değer, iç içe ve tek başına JSON string, dict örneği ve JSON yapısı önündeki kişisel sır/Bearer etiketi farklı canary'lerle denenir. Geçerli desteklenen girdide güvenli kardeş alanlar korunur; doğrulanamayan bozuk yapı görünür hatayla durur ve model/yayın üretmez. Canary ve bilinen JSON kaçışlı karşılıkları aşağıdaki bütün yüzeylerde aranır; özgün ek dosyasının bayt/hash kimliği değişmez. Sır içeren kullanıcı mesajı ile aynı değeri etiketsiz tekrarlayan Assistant yanıtı sonraki kalıcı hafıza girdisine alınmaz. Ayrıca sentetik çok satırlı PEM/sır, başlangıcı gerçek batch kesiminden önce, canary gövdesi ve bitişi sınırın ötesinde kalacak şekilde yerleştirilir; hedefin ölçülen batch sınırı ve transcript konumlarıyla kesişim kanıtlanır. Gerçek hook olaylarından sonra her canary, oluşan bütün parçalara ait arka plan özetleyici/derleyici model girdilerinde, kalıcı runtime state'in tamamında (kuyruk payload'ı, transcript-index, coverage, receipt, supervisor ve tanı dosyaları dahil), günlük/Companion/knowledge kayıtlarında ve filtreli görünümlerde bulunmaz. Ayrıca özetleme/derleme ve ayrı şema-onarım model çıktıları, App girdisinde bulunmayan farklı sentetik credential değerleriyle kontrollü üretilir; bu çıktı canary'leri de terfi eden günlük/knowledge ve bütün türev yüzeylerde süzülür. Ayrıca aynı gerçek App mesajında sentetik sır ve “Bu konuşmada kalsın” birlikte verilir; sır süzmesi gizlilik tercihini düşürmez. Session-only marker oluşur, o katkı ve sonraki olağan katkılar model/kalıcı hafızaya ulaşmaz. Asıl kullanıcı girdisinin bulunduğu ham transcript ve test için yakalanan ham model yanıtı, süzülmüş kalıcılık çıktılarından ayrı tutulur. |
| Şema onarımının dosya sınırı | Gerçek derleyici tek onarım turunda raporlanmış değişen bozuk dosyaları düzeltirken kontrollü model çıktısı ayrıca ilgisiz knowledge notunu değiştirmeye çalışır. Aynı ada sahip `knowledge/index.md` ve `knowledge/concepts/index.md` hedefleri ayrı koşulda sınanır. Yetki tam göreli dosya kimliğiyle sınırlıdır; bildirilen değişen hedeflerin başarılı toplu onarımı yayımlanabilir, ilgisiz dosyaya dokunan onarım görünür hata ile reddedilir ve canlı ilgisiz dosya baytları değişmez. Önce/sonra manifest, onarım kapsamı ve yayın sonucu birlikte doğrulanır. |
| Model çocuğunun ortam sınırı | Teste ait gerçek App/hook parent ortamına, konuşma/ek/model girdisinde bulunmayan ilgisiz credential canary'si konur. Gerçek özetleyici ve derleyici child ortamında bu değer bulunmaz; gerekli izinli bağlantı ayarları korunarak bağlantının çalıştığı doğrulanır. Sonuç değerleri ifşa edilmeden allowlist ve canary varlığı/yokluğu kaydedilir; parent ortamını tümüyle miras almak kabul edilmez. |
| Geç gelen oturuma özel tutma | Önce bir kayıt yayımlanır; sonra kullanıcı “bu konuşmada kalsın” der. Ajan mevcut `suppress_derived_memory` yoluyla geçmiş karşılıkları dışlar; kaynakları silmeden yeni oturum okuması ve derleme girdisinde dışlama doğrulanır. Session-only marker'ın yolu ve içeriği gerçek App session ID'sini taşımaz; yalnız geri çevrilemeyen kimlik kullanılır ve ham session ID iki yüzeyde de aranır. Yalnız session marker varlığı başarı değildir. |
...
| Ek dosyası işlenirken kapsam değişimi | Gerçek ek akışında read-only ve session-only kısıtları ayrı ayrı, model dönüşü ile kaynak notu yayını sınırlarında tetiklenir. Kabul edilmiş tercih sürümü ve yayın commit zamanı kaydedilir: tercih önce kabul edildiyse eski kapsamla not yayımlanamaz. Not atomik olarak önce tamamlandıysa bu sıralama kanıtlanır; session-only için mevcut geç-gelen dışlama yolu tamamlanıp notun sonraki hafıza kullanımından çıktığı doğrulanır. Read-only kabulünden sonra yeni yayın yapılmaz. Yarış yakalanamazsa kabul verilmez. |
| Özetleme sırasında salt okunur erteleme | Olağan transcript özeti modeldeyken read-only isteği kabul edilir; yayın/coverage ilerlemez ve hazırlanmış iş korunur. Sonraki açık uygulama isteği aynı kapsamı açınca, kaynak ve tercih yeniden doğrulanarak hazırlanmış sonuç yeni model çağrısı olmadan tam bir kez yayımlanır. Model çağrı sayısı, korunan iş, coverage ve receipt birlikte doğrulanır. |
...
| Salt okunur arama | Uzun, bağlantılı ve kısmen bastırılmış gerçek kaynak sorgulanır. Arama cache ve filtrelenmiş içerik dosyalarını değiştirmeden izin verilen içeriği sunar; ham kaynak path/link'i, gizli içerik veya bastırılmış kaynağa geri okuma yolu açılmaz. Ayrı yarış koşusunda filtreli tam görünüm hazırlandıktan sonra fakat görünüm/araç sonucu veya `additionalContext` döndürülmeden hemen önce kontrollü unutma/suppression güncellemesi uygulanır. Tercih sürümü tekrar doğrulanır; görünür hata ile kapalı kalır ve yeni yasaklanan metin iki çıktıda da bulunmaz. Yarışın zaman/sürüm ve çıktı kanıtı alınamazsa kabul açık kalır. Teknik sağlık metadata'sı içerik yazımıyla karıştırılmaz. |
| Knowledge yayını sırasında okuma | Gerçek SessionStart ve UserPromptSubmit koşularında knowledge kaynağı okunduktan sonra, çıktı dönmeden önce ayrı ayrı pending yayın journal'ı ve yeni tamamlanmış publication token'ı oluşturulur. Açılışta zaten pending olan yayın da ayrı sınanır. Olay, okunan nesil ve kapanış kontrolü eşleştirilir; görünür publication-pending/changed hatası oluşur, eski-yeni knowledge karışımı veya ham/cache fallback çıktısı verilmez. |
...
| Kapanış ve sıkıştırma | Gerçek host olayları alınır; `SessionEnd` matcher'ı gerçek kapanma nedeniyle eşleşir. Ayrıca App, anlamlı zararsız kullanıcı katkısını aldıktan fakat Stop/PreCompact/SessionEnd üretmeden önce beklenmedik biçimde kapanır. Sonraki startup, transcript dayanağı korunmuş katkıyı kaynak kimliğiyle bir kez kurtarır ve sonraki oturum onu kalıcı kaynaktan hatırlar. Olay, transcript, kurtarma ve tekilleştirme kanıtları eşleşmeden; katkı kayıpsa veya kurtarma doğrulanamıyorsa canlı kabul verilmez. Yalnız kapanış olayı olmadığını belgelemek yeterli değildir. Ayrı koşuda hook özgün sessions yolunu kaydettikten fakat worker okumadan önce test transcript'i hostun archived_sessions yoluna taşınır. Yalnız kaydedilmiş session kimliğine uyan arşiv dosyasıyla son katkı bir kez kurtarılır; var olan özgün kaynak, başka oturum veya sessions dışı ilgisiz yol arşiv fallback'iyle değiştirilmez. Girdi doğrulanmış başarı receipt'ine kadar korunur. |
| Native transcript içeriği ve rol ayrımı | Gerçek App transcript'inde AGENTS/plugin/environment enjeksiyon blokları, developer mesajları ve tool çıktıları kullanıcı/Assistant metniyle birlikte gözlenir. Yalnız gerçek konuşma metni model girdisine ve kaynak dayanağına geçer; enjeksiyon etiketleri gerçek kullanıcı metnini de silmez. Yalnız bilinen internal kayıtlardan oluşan transcript boş konuşma sayılır. Ayrı kontrollü koşularda bilinmeyen dolu zarf ve tanınan fakat metinsiz mesaj zarfı görünür unsupported-shape hatasıyla model/yayın/coverage ilerlemeden durur. Host zarf biçimi ve seçilen metin blokları kaynak testiyle eşlenir. |
| Transcript yazılırken yarım son kayıt | Gerçek App/host denemesinde, yalnız testin yönettiği transcript'in son JSON kaydı ve ayrı koşuda çok baytlı UTF-8 karakteri tamamlanmadan hook/kurtarma okuması tetiklenir. Okumanın yarım EOF aralığına denk geldiği olay ve bayt/hash kanıtıyla kaydedilir. Tamamlanan önek korunur; coverage ve receipt yarım kaydı onaylamaz. Yazıcı son kaydı bitirdikten sonra mevcut retry/kurtarma akışı katkıyı bir kez işler; tamamlanan önek tekrar yayımlanmaz ve son katkı kaybolmaz. Yarış yakalanamazsa veya son kayıt doğrulanamazsa kabul açık kalır. Ayrı koşularda daha sonraki geçerli satırdan önce newline ile tamamlanmış bozuk JSON ve tamamlanmış geçersiz UTF-8 kaydı yerleştirilir. Bunlar yarım EOF diye ertelenmez veya atlanmaz; görünür bozuk transcript hatası oluşur, model/yayın başlamaz ve coverage/receipt geçersiz kaydın ötesine ilerlemez. |
| Windows wrapper çıkışı ve yaşayan alt süreç | Yetkili Windows App/host denemesinde yalnız testin başlattığı zararsız wrapper ve alt süreç kullanılır; alt sürecin PID ve başlangıç kimliği kaydedilir. Wrapper alt süreçten önce çıkar, ardından gerçek timeout/cleanup yolu çalıştırılır. Cleanup talebi, tamamlanması ve doğrulanamaması ayrı kaydedilir; alt sürecin sınırlandırılmış süre içinde gerçekten sonlandığı PID/kimlik kontrolüyle doğrulanmadan kabul veya yeniden iş başlatma yoktur. İlgisiz sürecin sonlanması, yalnız wrapper'ın çıkışı veya beklemek başarı kanıtı sayılmaz. Ayrı koşuda gerçek başlatıcı, child oluşturulur oluşturulmaz çağırana dönüşten önce tutulur ve teste ait owner kontrollü kapatılır. Child'ın oluşturulurken Job Object sahipliğine girdiği ve owner ölünce süre sınırında sonlandığı PID/başlangıç kimliğiyle doğrulanır; sonradan atama aralığında sahipsiz child veya ikinci yazıcı bırakan akış kabul edilmez. |
...
| Kesinti ve kilit | Kuyruk kabulü gecikince kaynak/teslim girdisi korunur. Kapasite reddi ayrıca oluşturulur; reddedilen veya karantinadaki unresolved işin kurtarma girdisi temizleme/retention sonrasında durur ve kapasite açılınca katkı kaybolmadan işlenir. Kalıcı teslim veya receipt doğrulanmadan tek kurtarma kopyası temizlenmez. Stale hook-input temizliği ile yeni queue kabulü kontrollü iç içe geçirilir; referans taraması ve silmenin aynı queue kilidi altında kaldığı doğrulanır. Yeni kabul edilmiş işin tek girdisi silinmez ve iş süre sınırında yayın/receipt ile tamamlanır; yalnız kabulü geciktirmek yarış kanıtı sayılmaz. Yarım çok dosyalı yayın sağlıklı görünmez; sahipliği belirsiz süreç tekrar başlatılmaz. Teste ait eski child claim aşamasından önce tutulur; iş yeni generation altında yeniden sahiplenildikten sonra eski `claim_token` ile devam ettirilir. Gerçek child'ın claim reddi ve iş kaydının önce/sonra aynı kaldığı doğrulanır; eski child model/yayın başlatamaz ve ikinci yazıcı oluşmaz. |
| Bozuk kuyruk başı ve geçerli kardeş iş | Kuyruğun en eski pending kaydında ayrı koşularda geçersiz kind ve timestamp oluşturulur; yanında gerçek geçerli App katkısının işi bulunur. Supervisor bozuk payload'ı özgün bayt/hash'iyle koruyup görünür tanıyla karantinaya alır ve geçerli işi süre sınırında tamamlar. Tek bir bozuk kayıt bütün kuyruğu durduramaz; karantina, kaynak girdisi, geçerli kardeşin yayın/receipt'i birlikte doğrulanır. Ayrı koşularda karantina payload ve receipt yazıları kontrollü başarısız kılınır; görünür hata oluşur ve özgün pending kaydı byte-identical korunur. Doğrulanmış kalıcı karantina kopyası olmadan tek kurtarma girdisi taşınmaz veya silinmez. |
...
| Git checkpoint sırasında kullanıcı çalışması | Gerçek derleyici checkpoint akışında commit hazırlanırken ilgisiz kullanıcı dosyası stage edilir; yalnız makine çıktıları commit edilir, kullanıcı içeriği ve başarıyla alınmış stage korunur. Başarısız commit sırasında makine dosyasına gelen yeni staged/unstaged düzenlemeler de ayrı sınanır; eski index snapshot'ı bunları geri alamaz. Index kilidi stage girişimini reddederse bu görünür kaydedilir ve kilit kalkınca stage tekrar alınabilir; başkasının index.lock dosyası silinmez, kendi kilidi sızmaz. Ref güncellemesinden hemen önce ayrı test yazıcısı branch'i ilerletir: compare-and-swap ertelemeli, araya giren commit ezilmemelidir. Ref güncellemesi sonrası timeout, ayrıca sonraki commit oluşmuşken timeout koşularında doğrulanmış checkpoint başarı sayılır ve index'e geri alma stage edilmez; ref sonucu belirsizse başarı verilmeden kurtarılabilir makine snapshot'ı korunur. Zararsız reddeden Git hook ve imzalama politikası ayrı koşularda checkpoint'i durdurur; politika atlanmaz. HEAD, index, çalışma dosyaları ve sonuç kodları birlikte karşılaştırılır; yalnız testin yönettiği hedefler kullanılır. |
| Büyük girdi ve korunmuş ek | Uzun koşullu ifade ve büyük sentetik satır yanlış başarıya veya eksik iddiaya dönüşmez. Gerçek test transcript'ine önce yayınlanabilir katkı, ardından içinde “Bu konuşmada kalsın” direktifi bulunan aşırı büyük satır eklenir. İşlem ilk katkı seçilmeden durur; model çağrısı, yayın ve coverage ilerlemez, satır boyutu hatası görünür kaydedilir. Ayrıca gerçek App'te batch sınırını aşan zararsız bir ek zarfı, özgün ek metaverisi ve kullanıcı isteğiyle birlikte verilir. Capture'a iletilen tam zarf özgün girdiyle karşılaştırılır; ek ve istek parça sınırında ayrılmaz. Sonraki arka plan model girdisinin beklenen izinli ek içeriği ve isteği kapsadığı doğrulanır; kısmi capture/işleme veya eşleşmeyen girdi kabulü açık bırakır. Geçici özgün eki artık bulunmayan korunmuş ek yalnız doğrulanmış kaynak eşlemesiyle yeniden okunur; içerik değişimi ve güncel gizlilik tercihi denetlenir. |
| Aynı ek kimliği, ayrı zarflar | Gerçek App'te aynı attachment UUID/path, farklı kullanıcı isteği taşıyan iki ayrı zarfla eşzamanlı kullanılır; ardından iki zarf ayrı ayrı replay edilir. Zarf hash'ine bağlı iki mapping/receipt birbirini ezmez, istekler yanlış ilişkilendirilmez; aynı değişmemiş kaynak gereksiz yeniden özetlenmez. İlk capture sonrası özgün geçici kaynak kaldırıldığında da her zarf yalnız kendi doğrulanmış eşlemesiyle korunmuş notu bulur. Olay sırası, iki zarf kimliği, model çağrı sayısı ve yayın bağlantıları birlikte doğrulanır. |
| Ek okumasının kök sınırı | Doğru Vault olay köküyle, izinli attachment kökü dışındaki teste ait canary dosyasına sahte ek zarfı yöneltilir. Kaynak okunmuş gibi başarı verilmez; görünür ret/tanı oluşur, canary model girdisine veya kayda ulaşmaz ve kaynak baytları değişmez. Bu koşul yanlış Vault kökü testinden ayrı yürütülür. |
| Ek mapping'i ile kaynak notu arasındaki kesinti | Mapping hazırlandıktan sonra kaynak notu yazılmadan kontrollü kesinti oluşturulur. Retry, tek doğrulanmış kaynak notu ve doğru zarf bağlantılı yayın üretir; mapping/receipt kaynak veya özetin düz metnini taşımaz. Özgün kaynak ve doğrulanmış not birlikte yoksa başarıya geçilmez. Mevcut retry koşularından biri tarih sınırını aşar; korunmuş hazırlanmış özet ve yayın özgün olay gününe bağlı kalır. |
| Başarısız kayıt | Gerçek unresolved terminal hata oluşturulur; sonraki gerçek App oturumu açıldığında aynı çözülmemiş kaydın ve kaydedilemeyen katkının tanısı hâlâ görünürdür. Kapanış hook'unda bir kez gösterilmiş olması yeterli değildir. Ayrı kurtarma koşusunda yalnız kalıcı başarılı successor/receipt ile eşleşen eski hata uyarısı bastırılır; böyle bir kanıt yokken kayıt kaybolmaz veya kurtarıldı sayılmaz. Retention çalıştırıldıktan sonra da eski terminal kaydı çözen gerekli successor/receipt kanıtı korunur; çözülmüş hata kanıt silindiği için yeniden kayıp gibi gösterilmez. |
