...
| Tekrarlı olay ve aynı içerikli ayrı katkılar | Aynı transcript aralığının Stop, PreCompact ve SessionEnd replay'i ek yayın üretmez. Ayrı koşuda, içeriği birebir aynı olan iki meşru ardışık batch gerçek App transcript'inde farklı konumlarla oluşturulur. Her konum bir kez işlenir ve coverage ikisinin sonuna kadar ilerler; yalnız içerik hash'iyle ikinci katkı atılmaz. Coverage yalnız mevcut transcript önekinin doğrulanmış bayt/hash kimliğiyle geçerlidir; aynı uzunluktaki önek değişiminde eski coverage yeni baytlara aktarılmaz, transcript kısaltılınca da taşınmaz; yeni katkı yeniden işlenir veya işlem güvenli biçimde durur. Sonraki lifecycle replay'i üçüncü yayın üretmez; konum, coverage ve receipt kanıtları birlikte doğrulanır. |
...
| Kesinti ve kilit | Kuyruk kabulü gecikince kaynak/teslim girdisi korunur. Kalıcı teslim veya receipt doğrulanmadan tek kurtarma kopyası temizlenmez. Yarım çok dosyalı yayın sağlıklı görünmez; sahipliği belirsiz süreç tekrar başlatılmaz. Yeni nesil işi yeniden sahiplendikten sonra eski neslin `claim_token`'ını taşıyan child reddedilir; iş kaydı değişmez ve ikinci yazıcı oluşmaz. |
...
| Büyük girdi ve korunmuş ek | Uzun koşullu ifade ve büyük sentetik satır yanlış başarıya veya eksik iddiaya dönüşmez. Transcript'te önce yayınlanabilir satır bulunsa bile, `session-only` içeren aşırı büyük satır seçimi öncesinde durur; model çağrısı, yayın ve coverage ilerlemez, satır boyutu hatası görünür kaydedilir. Ayrıca gerçek App'te batch sınırını aşan zararsız bir ek zarfı, özgün ek metaverisi ve kullanıcı isteğiyle birlikte verilir. Capture'a iletilen tam zarf özgün girdiyle karşılaştırılır; ek ve istek parça sınırında ayrılmaz. Sonraki arka plan model girdisinin beklenen izinli ek içeriği ve isteği kapsadığı doğrulanır; kısmi capture/işleme veya eşleşmeyen girdi kabulü açık bırakır. Geçici özgün eki artık bulunmayan korunmuş ek yalnız doğrulanmış kaynak eşlemesiyle yeniden okunur; içerik değişimi ve güncel gizlilik tercihi denetlenir. |
